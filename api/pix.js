...
export default async function handler(req, res) {
  ...
  // ------------------------------------------------------------------
  // 2. NOVA ROTA: VERIFICAR STATUS DO PAGAMENTO (GET)
  // ------------------------------------------------------------------
  if (req.method === 'GET') {
    const { id } = req.query;
    ...
    return new Promise((resolve, reject) => {
      const mpReq = https.request(options, (mpRes) => {
        ...
        mpRes.on('end', () => {
          try {
            ...
            // Retorna status e detalhes para o Frontend decidir
            res.status(200).json({ 
                status: json.status, 
      ...
      mpReq.on('error', (e) => {
        res.status(500).json({ error: 'Erro de conexão na verificação' });
        resolve();
      });
  ...
  // ------------------------------------------------------------------
  // 3. ROTA ORIGINAL: CRIAR PAGAMENTO (POST)
  // ------------------------------------------------------------------
  if (req.method === 'POST') {
    try {
        const body = req.body;
        ...
        // Define se é empresa ou pessoa
        const entityType = (docType === 'CNPJ') ? 'association' : 'individual';
        const payerEmail = (body.payer && body.payer.email) ? body.payer.email : 'cliente_nexlog@email.com';
        ...
        res.status(500).json({ error: 'Erro crítico backend', details: err.message });
    }
  }
