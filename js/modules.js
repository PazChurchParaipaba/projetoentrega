    ...
    store: {
        init: async () => {
            ...
            if (lojaHeaderInfo) {
                ...
                <p style="margin:0; font-size:1.2rem; font-weight:600; color:var(--primary);">${store.nome_loja || 'Loja Nova'}</p>
                <p style="margin:0; font-size:0.9rem; color:var(--text-muted);">Ramo: ${store.tipo_loja || 'Geral'} | CNPJ: ${store.cnpj || 'Não informado'}</p>
            `;
            }
        ...
        monitorKitchen: () => {
            ...
            _sb.channel('cozinha-mesas').on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'comandas', filter: 'imprimir_cozinha=eq.true' }, async (payload) => {
                if (payload.new.store_id === App.state.storeId) {
                    const { data } = await _sb.from('comandas').select('*').eq('id', payload.new.id).single();
                    if (data && data.items && data.items.length > 0) {
                        let itensParaImprimir = [];
                        let novosItens = data.items.map(item => { const diff = (item.qtd || 0) - (item.printed_qtd || 0); if (diff > 0) { itensParaImprimir.push({ ...item, qtd: diff }); return { ...item, printed_qtd: item.qtd }; } return item; });
                        if (itensParaImprimir.length > 0) {
                            const itemsHtml = itensParaImprimir.map(i => `<div style="border-bottom:1px dashed #000; padding:5px 0;"><div style="font-size:1.2rem; font-weight:bold;">${i.qtd}x ${i.nome}</div>${i.obs ? `<div style="font-size:0.9rem;">⚠️ OBS: ${i.obs}</div>` : ''}<div style="font-size:0.7rem; text-align:right;">Garçom: ${i.garcom || 'Geral'}</div></div>`).join('');
                            const content = `<div style="font-family: monospace; width: 100%; padding: 0 5px;"><br><h3 style="text-align:center; margin:0;">PEDIDO SALÃO 🍽️</h3><div style="border: 2px solid black; margin: 5px 0; padding: 5px;"><p style="text-align:center; font-size:2rem; font-weight:800; margin:0;">MESA ${data.numero}</p></div><p style="text-align:center;">${new Date().toLocaleTimeString()}</p><hr style="border-top: 2px solid black;">${itemsHtml}<hr style="border-top: 2px solid black;"><br>.</div>`;
                            if (printContent(content)) { await _sb.from('comandas').update({ imprimir_cozinha: false, items: novosItens }).eq('id', payload.new.id); }
                        } else { if (localStorage.getItem('NAXIO_AUTO_PRINT') === 'true') { await _sb.from('comandas').update({ imprimir_cozinha: false }).eq('id', payload.new.id); } }
                    }
                }
            }).subscribe();
        },
        ...
        checkCategory: (c) => {
            ...
            // NOVA LÓGICA: Se for Comida ou Bebida, mostra config de Impressora e Taxa
            if (c === 'Comidas' || c === 'Bebidas') {
                    ...
                    <div class="modal-body">
                        <div class="input-wrapper">
                            ...
                            <div style="display:flex; gap:10px;">
                                <button class="btn btn-secondary" onclick="const el=document.getElementById('w-qtd'); el.value=Math.max(1, parseInt(el.value)-1)">-</button>
                                <input type="number" id="w-qtd" class="input-field" value="1" style="text-align:center;">
                                <button class="btn btn-secondary" onclick="const el=document.getElementById('w-qtd'); el.value=parseInt(el.value)+1">+</button>
                            </div>
                        </div>
                        <div class="input-wrapper">
...
App.store.showNotasNaoEmitidasModal = async () => {
    ...
    const lista = notas.map(n => `
       <div style="display:flex; justify-content:space-between; align-items:center; padding: 10px; border-bottom: 1px solid var(--border);">
           <div>
               <strong>Pedido #${n.id.slice(0, 6)}</strong> - R$ ${n.total_pago}<br>
               <small class="text-muted">${new Date(n.created_at).toLocaleString()}</small>
           </div>
           <button class="btn btn-sm btn-primary" onclick="App.fiscal.emitirNFCe('${n.id}', ${n.total_pago}, []); this.parentElement.remove();">Emitir</button>
       </div>
   `).join('');

    // Criação manual do modal (Genérico) para evitar erro de dependência
    const m = document.createElement('div');
    m.className = 'modal-overlay';
    m.style.cssText = 'display:flex; z-index:10010; align-items:center; justify-content:center;';
    m.innerHTML = `<div class="modal-content" style="background:var(--surface); width:90%; max-width:400px; color:var(--text-color); border:1px solid var(--border);">
           ...
           <div class="modal-body" style="background:var(--surface);">${lista}</div>
       </div>`;
            ...
            <div class="modal-content" style="max-width:400px; width:90%; background:var(--surface); color:var(--text-color); border:1px solid var(--border);">
                ...
                <div class="modal-body">
                    <div style="background:rgba(59, 130, 246, 0.1); padding:10px; border-radius:6px; margin-bottom:15px; border:1px solid var(--info);">
                        <p class="text-xs" style="color:var(--info); margin:0;">Isso adicionará o produto em <strong>TODAS as mesas ABERTAS</strong> no intervalo informado.</p>
                    </div>
...
App.store.confirmarLancamentoLote = async () => {
    ...
    if (comandas.length === 0) {
        alert("Nenhuma mesa ABERTA encontrada neste intervalo (Números " + inicio + " a " + fim + ").");
        return;
    }
    ...
    for (const comanda of comandas) {
        ...
        items.push(novoItem);
        // Atualiza Supabase
        await _sb.from('comandas').update({ items, updated_at: new Date().toISOString() }).eq('id', comanda.id);
        atualizados++;
    }
