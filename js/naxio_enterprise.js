...
const NaxioEnterprise = {
    ...
    stockHistory: {
        ...
        addManualEntry: async () => {
            ...
            const prodName = prompt('Nome ou código do produto:');
            if (!prodName) return;
            ...
            const qtd = parseInt(prompt(`Quantidade de ENTRADA para "${prod.nome}":`));
            if (!qtd || qtd <= 0) return;

            const motivo = prompt('Motivo (Ex: Compra fornecedor, Devolução):') || 'Entrada manual';
        ...
        addAdjustment: async () => {
            const prodName = prompt('Nome ou código do produto para ajustar:');
            if (!prodName) return;
            ...
            const novoQtd = parseInt(prompt(`"${prod.nome}" - Estoque atual: ${prod.estoque || 0}\nDigite o estoque correto:`));
            if (novoQtd === null || isNaN(novoQtd)) return;
