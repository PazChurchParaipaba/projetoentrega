...
const Caixa = {
        ...
        container.innerHTML = `
            <div class="caixa-panel">
                ...
                <div class="grid grid-3">
                    <div class="stat-box">
                        <h5>Fundo Inicial</h5>
                        <div class="stat-val" style="color:#64748b;">R$ ${session.valor_inicial.toFixed(2)}</div>
                    </div>
                    <div class="stat-box">
                        <h5>Vendas do Dia</h5>
            ...
            <div id="ticket-area" class="thermal-print"></div>
        `;
        Caixa.loadMovements();
    },
    ...
    calcTotals: async () => {
        ...
        // 1. Busca VENDAS (Orders) - Filtra Interna
        const { data: sales } = await _sb.from('orders')
            .select('*')
            .eq('store_id', App.state.storeId)
            .eq('session_id', Caixa.state.session.id)
            .eq('status', 'concluido')
            .neq('origem_venda', 'interna'); // 🔥 NÃO SOMA INTERNA
        ...
        if (sales) {
            ...
            sales.forEach(o => {
                ...
                methods.forEach(p => {
                    ...
                    } else if (t.includes('crédito') || t.includes('credit')) {
    ...
    // 🔥 FECHAMENTO CORRIGIDO COM DETALHAMENTO
    closeSession: async () => {
        ...
        const diferenca = contado - esperadoGaveta;

        const resumoCompleto = { ...st, esperadoGaveta, contado, diferenca, fundo };

        // 1. Imprime via thermal (window.print)
        const content = Caixa.generateContent(resumoCompleto);
        document.getElementById('ticket-area').innerHTML = content;
        setTimeout(() => window.print(), 300);

        // 2. 🔥 GERA PDF PARA DOWNLOAD com nome VENDAS (DATA)
        const nomeArquivo = `VENDAS ${dataFechamento.toLocaleDateString('pt-BR').replace(/\//g, '-')}.pdf`;
        setTimeout(() => Caixa.gerarPdfFechamento(resumoCompleto, nomeArquivo), 800);
        ...
        if (error) alert("Erro ao fechar: " + error.message);
            ...
            setTimeout(() => {
                ...
                Caixa.state.session = null;
                localStorage.removeItem('active_cash_session');
                Caixa.checkSession().then(() => Caixa.render());
            }, 1200);
        }
    },
