...
const GarcomSystem = {
    ...
    state: {
        ...
        searchQuery: '',
        searchTableQuery: '', // Nova variável de busca para mesas/comandas
    ...
    data: {
        ...
        syncTables: async () => {
            ...
            // Explicitly excluding 'fechada' and 'arquivada' and 'paga'
            const { data, error } = await _sb
                .from('comandas')
                .select('*')
                .eq('store_id', GarcomSystem.state.storeId)
        ...
        sendOrder: async (tableNum, items, obsGeral = "") => {
            if (GarcomSystem.state._isSendingOrder) return;
            GarcomSystem.state._isSendingOrder = true;

            if (GarcomSystem.ui.showLoading) GarcomSystem.ui.showLoading("🚀 Enviando para cozinha...");
            ...
            try {
                // Stampa os itens com flags de impressão
                const stampedItems = items.map(i => ({
                    ...i,
                    garcom: GarcomSystem.state.waiterName,
                    data_lancamento: new Date().toISOString(), // 🚀 NOME CORRETO PARA O AGENTE
                    printed: false,
                    printed_qtd: 0,
                    status: 'sent'
                }));

                // 1. Busca apenas o ID da comanda (sem SELECT de items — mais rápido)
                const { data: extComanda } = await _sb.from('comandas')
                    .select('id')
                    .eq('store_id', GarcomSystem.state.storeId)
                    .eq('numero', tableNum)
                    .neq('status', 'arquivada')
                ...
                if (extComanda) {
                    comandaId = extComanda.id;

                    // 2a. ✅ APPEND ATÔMICO via RPC (PostgreSQL faz o merge no servidor)
                    // Zero race condition: mesmo 10 garçons ao mesmo tempo não perdem itens.
                    // A função append_comanda_items usa || (JSONB concat) diretamente no UPDATE.
                    const { error } = await _sb.rpc('append_comanda_items', {
                        p_comanda_id: comandaId,
                        p_new_items: stampedItems,
                        p_obs_geral: obsGeral || null
                    });

                    if (error) throw error;

                    // 3. 🚨 SINALIZA O AGENTE — Força a flag de impressão para True
                    await _sb.from('comandas').update({ imprimir_cozinha: true }).eq('id', comandaId);

                } else {
                    // 2b. Comanda nova: INSERT com imprimir_cozinha=true
                    const { data: newComanda, error: createError } = await _sb.from('comandas').insert({
                ...
                // 3. UI responde IMEDIATAMENTE — syncTables em background
                GarcomSystem.ui.toast(`✅ Mesa ${tableNum} — Pedido na cozinha!`, "success");
                GarcomSystem.state.cart = [];
                GarcomSystem.ui.closeModal();

                GarcomSystem.data.syncTables(); // sem await — não bloqueia o garçom
                return comandaId;

            } catch (err) {
                console.error("Erro sendOrder:", err);
                GarcomSystem.ui.toast("Erro ao enviar pedido: " + (err.message || 'Tente novamente'), "error");
                return null;
            } finally {
                GarcomSystem.state._isSendingOrder = false;
                if (GarcomSystem.ui.hideLoading) GarcomSystem.ui.hideLoading();
            }
        },
    ...
    realtime: {
        ...
        handleUpdate: (payload) => {
            ...
            if (eventType === 'INSERT' || eventType === 'UPDATE') {
                ...
                // Se a mesa mudou para fechada, paga ou arquivada, removemos da lista do garçom
                if (['fechada', 'arquivada', 'paga'].includes(newRec.status)) {
                    if (idx >= 0) GarcomSystem.state.tables.splice(idx, 1);
                    // Se o garçom estiver com a mesa aberta na tela, avisar e fechar
                    if (GarcomSystem.state.activeTable === newRec.numero && GarcomSystem.state.currentView === 'table') {
                        GarcomSystem.ui.toast(`Mesa ${newRec.numero} bloqueada/recebida no caixa!`, "warning");
                        GarcomSystem.router.navigate('home');
                    }
                } else {
                    if (idx >= 0) {
                        ...
                        // Se não existia (era INSERT ou estava fechada e foi reaberta), adiciona
                        GarcomSystem.state.tables.push(newRec);
                    }
                }
            }
    ...
    ui: {
            ...
            content.innerHTML = `
                <div style="padding: 15px 15px 0 15px;">
                    <div style="position:relative;">
                        ...
                        <input type="number" id="comanda-search" class="g-input" placeholder="Buscar Nº da Comanda..." 
                            value="${GarcomSystem.state.searchTableQuery || ''}"
                            style="padding-left:40px; margin:0;"
                            oninput="GarcomSystem.state.searchTableQuery = this.value; GarcomSystem.ui.updateTableStatuses()">
                    </div>
                </div>
        ...
        updateTableStatuses: () => {
            if (GarcomSystem.state._updateTimer) clearTimeout(GarcomSystem.state._updateTimer);
            GarcomSystem.state._updateTimer = setTimeout(() => {
                const container = document.getElementById('table-grid-area');
                if (!container) return;

                // Filtra as comandas baseando-se no campo searchTableQuery
                let tablesToRender = GarcomSystem.state.tables;
                const query = (GarcomSystem.state.searchTableQuery || '').trim();
                if (query) {
                    tablesToRender = tablesToRender.filter(t => String(t.numero).includes(query));
                }

            // Always re-build to ensure state sync, replacing Skeletons or Old data
            // Optimized HTML generation
            container.innerHTML = tablesToRender.map(t => {
                ...
                if (tablesToRender.length === 0) {
                    container.innerHTML = `<div style="text-align:center; grid-column: 1 / -1; padding:20px; color:var(--g-text-muted);">Nenhuma comanda encontrada.</div>`;
                }
            }, 50); // Debounce de 50ms para evitar travamento
        },
                            ...
                            <div class="g-card anim-slide" style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:8px; padding:12px;">
                                <div style="flex:1;">
                                    ...
                                    <div style="font-size:0.75rem; color:var(--g-text-muted); margin-top:4px;">
                                        ${(item.added_at || item.data_lancamento) ? new Date(item.added_at || item.data_lancamento).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }) : '--:--'} • ${item.garcom || 'Staff'}
                                    </div>
                                </div>
                ...
                <div class="g-modal-sheet" style="height:auto;">
                    <div style="padding:20px;">
                        ...
                        <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px; margin-top:20px;">
                            ...
                            <div class="g-card" onclick="if(window.ReservasPratosSystem) { ReservasPratosSystem.abrirPainel(); } else { alert('Módulo de Reservas não carregado.'); }" style="text-align:center; cursor:pointer;">
                                <i class="ri-calendar-check-line" style="font-size:2rem; color:#a855f7;"></i>
                                <div style="margin-top:5px;">Reservas</div>
                            </div>
                            <div class="g-card" onclick="if(window.ReservasPratosSystem) { ReservasPratosSystem.abrirPainel(); } else { alert('Módulo de Reservas não carregado.'); }" style="text-align:center; cursor:pointer;">
                                <i class="ri-calendar-check-line" style="font-size:2rem; color:#a855f7;"></i>
                                <div style="margin-top:5px;">Reservas</div>
    ...
    orders: {
        ...
        renderProducts: (category, query = null) => {
            ...
            if (query) {
                ...
                products.sort((a, b) => {
                    const cA = String(a.codigo_barras || a.codigo_cardapio || a.cod || (a.id && a.id.slice(0, 4)) || '').toLowerCase();
                    const cB = String(b.codigo_barras || b.codigo_cardapio || b.cod || (b.id && b.id.slice(0, 4)) || '').toLowerCase();
                    const matchA = cA.includes(q) ? 1 : 0;
                    const matchB = cB.includes(q) ? 1 : 0;
            ...
            modal.innerHTML = `
                <div class="g-modal-sheet" style="height:auto; border-radius:16px;">
                    <div style="padding:20px;">
                        ...
                        <label style="display:block; margin-bottom:10px; font-size:0.9rem; color:#aaa;">📝 Número da Mesa (Para vincular antes)</label>
                        <input id="item-mesa-input" type="number" class="g-input" placeholder="Ex: 5" value="${GarcomSystem.state.activeTable || ''}">
        ...
        confirmAddItem: (id) => {
            ...
            const mesaInput = document.getElementById('item-mesa-input');
            if (mesaInput && mesaInput.value) {
                obs = "MESA " + mesaInput.value + (obs ? " - " + obs : "");
            }
            ...
            const item = {
                ...
                impressora_alvo: product.impressora_alvo || 'PADRAO',
                data_lancamento: new Date().toISOString(),
                added_at: new Date().toISOString(),
                garcom: GarcomSystem.state.waiterName || 'Staff',
                status: 'new'
            };
        ...
        reviewOrder: (tableNum) => {
            ...
            // Verifica se está no horário de reserva para mostrar o botão destacado
            const isHorarioReserva = true;
            ...
            modal.innerHTML = `
                <div class="g-modal-sheet">
                    <div class="modal-header" style="padding:15px; border-bottom:1px solid #333; display:flex; align-items:center; gap:12px;">
                        <i class="ri-arrow-left-line" style="font-size:1.5rem; cursor:pointer; color:var(--g-primary);" onclick="this.closest('.g-modal-overlay').remove()"></i>
                        <h3 style="margin:0;">Revisar Pedido - Mesa ${tableNum}</h3>
                    </div>
                    ...
                    <div class="modal-footer" style="padding:15px; border-top:1px solid #333;">
                        ...
                        <!-- BOTÃO DE RESERVA: visível sempre no horário, oculto fora -->
                        <div id="reserva-toggle-wrap" style="margin-bottom:12px; ${isHorarioReserva ? '' : 'display:none;'}">
                            <button
                                id="btn-reserva-toggle"
                                data-ativo="false"
                                onclick="GarcomSystem.orders.toggleReserva()"
                                style="
                                    width:100%; padding:14px; border-radius:12px; border:2px dashed #f59e0b;
                                    background:transparent; color:#f59e0b; font-size:1rem; font-weight:700;
                                    cursor:pointer; transition:all 0.2s;
                                "
                            >
                                📅 Marcar como RESERVA de Prato?
                            </button>
                        </div>

                        <div style="display:flex; justify-content:space-between; margin:10px 0 15px; font-size:1.2rem; font-weight:bold;">
                            <span>Total</span>
        ...
        toggleReserva: () => {
            const btn = document.getElementById('btn-reserva-toggle');
            if (!btn) return;
            ...
            if (!ativo) {
                ...
                btn.textContent = '✅ RESERVA DE PRATO MARCADA — clique p/ desmarcar';
            } else {
                // Desativando
                btn.style.background = 'transparent';
                btn.style.color = '#f59e0b';
                btn.style.border = '2px dashed #f59e0b';
                btn.textContent = '📅 Marcar como RESERVA de Prato?';
            }
        },
        ...
        submitFinal: async (originalTableNum) => {
            if (GarcomSystem.state._isSubmitting) return;

            const btnEnvio = document.querySelector('button[onclick*="submitFinal"]');
            if (btnEnvio) {
                btnEnvio.disabled = true;
                btnEnvio.innerHTML = '<i class="ri-loader-4-line anim-spin"></i> ENVIANDO...';
            }

            GarcomSystem.state._isSubmitting = true;

            try {
                const tableNum = originalTableNum;
                let obsGeral = document.getElementById('order-obs-geral').value;
                GarcomSystem.ui.setLoading(true);

                // 1. Captura itens ANTES que o sendOrder limpe o carrinho global
                const cartItems = [...GarcomSystem.state.cart];

                // Lê o estado do botão de reserva do modal (sem usar confirm())
                const btnReserva = document.getElementById('btn-reserva-toggle');
                const isReserva = btnReserva ? btnReserva.dataset.ativo === 'true' : false;

                // Se for reserva, seta flag is_reserva:true nos itens (para o agente.py detectar)
                // e adiciona [RESERVA] na observação para visibilidade
                if (isReserva) {
                    cartItems.forEach(item => {
                        item.is_reserva = true;
                        item.observacao = item.observacao ? `[RESERVA] ${item.observacao}` : `[RESERVA]`;
                    });
                }

                // O sendOrder cria a comanda e atualiza os items
                const comandaId = await GarcomSystem.data.sendOrder(tableNum, cartItems, obsGeral);
                ...
                // Se for reserva, salva também na tabela de reservas_pratos
                if (isReserva && comandaId) {
                    const reservasLote = cartItems.map(item => ({
                        store_id: GarcomSystem.state.storeId,
                    ...
                    if (!reservaError) {
                        console.log("✅ Reserva salva na tabela 'reservas_pratos' com sucesso!");
                        GarcomSystem.ui.toast(`✅ Reserva de ${cartItems.length} item(s) registrada!`, 'success');
                    } else {
                        console.error("Erro ao salvar reserva:", reservaError);
                        GarcomSystem.ui.toast("Pedido enviado, mas falha ao salvar reserva: " + reservaError.message, 'warning');
                    }
                }
                ...
                GarcomSystem.state._isSubmitting = false;
                if (btnEnvio) {
                    btnEnvio.disabled = false;
                    btnEnvio.innerHTML = '🚀 ENVIAR PEDIDO';
                }
            }
        }
    },
    ...
    bills: {
                ...
                <div class="g-modal-sheet" style="height:auto; min-height:50vh;">
                    <div style="padding:20px; text-align:center;">
                        ...
                        <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px; margin-top:20px;">
                            ...
                            <button class="g-btn ghost" onclick="GarcomSystem.bills.showSplit(${tableNum}, ${total * 1.1})">
        ...
        print: async (tableNum) => {
            ...
            // Formata OBS GERAL (RESERVA)
            const obsHtml = t.obs_geral ?
                `<div style="text-align:center; margin-bottom:15px; padding:10px; border:2px dashed #fff; font-weight:bold; font-size:1.1rem; ${t.obs_geral.includes('RESERVA') ? 'color:var(--g-warning); font-size:1.5rem; border-color:var(--g-warning);' : ''}">
                    ${t.obs_geral.replace(/\n/g, '<br>')}
                </div>` : '';
                        ...
                        <div style="display:flex; flex-direction:column; gap:8px;">
                            ${printers.length > 0 ? printers.map(p => `
                                <button class="g-btn ghost" onclick="GarcomSystem.bills.sendToPrinter('${p.id}', ${tableNum})" style="justify-content:space-between;">
                                    <span><i class="ri-printer-line"></i> ${p.nome || p.name || 'Impressora '}</span>
        ...
        showSplit: (num, total) => {
            const pessoas = prompt("Dividir a conta (com 10%) para quantas pessoas?", "2");
            if (pessoas && !isNaN(pessoas)) {
                const par = total / parseInt(pessoas);
                alert(`Total com 10%: R$ ${total.toFixed(2)}\nFica R$ ${par.toFixed(2)} para cada (${pessoas} pessoas)`);
            }
        },
    ...
    management: {
        ...
        confirmTransfer: async (from, to) => {
            ...
            const senha = prompt("🔒 Digite a senha de autorização (ex: naxio123 ou gerente123):");
            if (senha !== 'naxio123' && senha !== 'gerente123' && senha !== '1234') {
            ...
            try {
                ...
                if (vendas) {
                    vendas.forEach(v => {
                        ...
                        // Tenta pela coluna explícita, senão compara o pago com o subtotal (para pedidos antigos)
                        const subtotalOrder = parseFloat(v.total || 0);
                        const pagoOrder = parseFloat(v.total_pago || 0);
                        const temTaxa = (parseFloat(v.taxa_servico) > 0) || (pagoOrder > (subtotalOrder + 0.05));
                        ...
                        if (servedThisTable) {
                            tablesServed++;
                            // 🔥 Agora o total global reflete o VALOR PAGO (Bruto) da comanda para bater com o Caixa
                            globalTotalSales += parseFloat(v.total_pago || 0);
                        }
                    });
                }
                ...
                const waitersListHTML = Object.values(waitersMap).sort((a, b) => b.total - a.total).map(w => {
                    ...
                    <div style="margin-bottom: 8px;">
                        ...
                        <div style="display: flex; justify-content: flex-end; font-size: 1.3rem; margin-top:2px;">
                            <span style="font-weight: 900;">Comissão: R$ ${w.commission.toFixed(2)}</span>
                        </div>
                    </div>
                ...
                const reportContent = `
                    <div style="font-family: 'Courier New', Courier, monospace; color: black; text-align: left; max-width: 350px; margin: 0 auto; background: white; padding: 10px; font-weight: 900;">
                        ...
                        <div style="text-align: center; font-weight: 900; font-size: 1.4rem; margin-bottom:5px;">RELATORIO COMISSOES</div>
        ...
        filter: () => {
            ...
            if (query) {
                ...
                products.sort((a, b) => {
                    const cA = String(a.codigo_barras || a.codigo_cardapio || a.cod || (a.id && a.id.slice(0, 4)) || '').toLowerCase();
                    const cB = String(b.codigo_barras || b.codigo_cardapio || b.cod || (b.id && b.id.slice(0, 4)) || '').toLowerCase();
                    const matchA = cA.includes(query) ? 1 : 0;
                    const matchB = cB.includes(query) ? 1 : 0;
                    if (matchA > matchB) return -1;
                    if (matchB > matchA) return 1;
                    return (a.nome || '').localeCompare(b.nome || '');
                });
